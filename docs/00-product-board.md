...
|---|---|---|---|---|
| B-041 | **⚠️ Filou a laisse tomber le mardi SANS LE DIRE.** MiKL demande « lundi ET mardi, semaines paires » ; la proposition ne porte que le lundi, et rien ne signale l'autre moitie. **Cause identifiee, et elle est de MOI** : le prompt impose « un seul outil de modification par reponse » (`agentFilou.ts:163`), et j'ai decide le 26/08 « un seul jour par regle » (B-038) — Filou ne POUVAIT donc pas honorer la demande entiere. Le defaut n'est pas la limite, c'est le SILENCE : il a choisi une moitie et presente le resultat comme complet. **Bonne nouvelle : la limite n'a pas lieu d'etre.** Les deux gardiens bouclent DEJA sur plusieurs entrees (`for (const regle of cfg.regles)` — hard-constraints.ts:203, validerPlanning.ts:690). Une regle « lundi + mardi, semaines paires » est evaluable des aujourd'hui ; seule la saisie ne sait pas l'ecrire. Meme famille que B-005 et B-019 : **le risque n'est jamais la capacite manquante, c'est la reponse partielle presentee comme complete.** | MiKL | 2026-08-26 | A qualifier |
| B-042 | **Un conge « tous les lundis, tout le temps » est une REGLE, et Filou ne le voit pas.** MiKL : « mets-moi des conges le lundi et mardi semaine paire » → « il me faut des dates precises ». Techniquement juste (un conge est date), mais c'est une impasse : la demande n'a pas de fin, donc aucune date ne la satisfait. Filou devrait poser LA question qui tranche — « ponctuel, sur des dates precises, ou permanent ? » — et, si c'est permanent, proposer la regle correspondante. Il finit par le faire, mais seulement apres que MiKL a devine tout seul qu'il fallait dire « c'est tout le temps ». **L'assistant doit porter la traduction metier, pas la faire deviner.** | MiKL | 2026-08-26 | A qualifier |
...
|---|---|---|---|
| B-040 | Filou sait qui dit « je » | `ContexteOutil` porte desormais `prenom` et `nom` (il n'avait que l'identifiant, inutilisable dans une phrase). Lus dans `filou/actions.ts` et dans le banc d'essai | OK |
| B-040 | Il le sait DANS SA TETE, pas seulement dans le contexte | Bloc « QUI TE PARLE » ajoute au prompt : nom, role, et la consigne explicite « ne demande JAMAIS son prenom : tu l'as » (`agentFilou.ts`) | OK |
| B-040 | Sans casser la mise en cache du prompt | Bloc SEPARE et NON cache, place APRES le bloc cache — le cache porte sur un prefixe. Fondre l'identite dans le prompt principal l'aurait fait manquer a chaque conversation, pour tout le monde | OK |
| B-040 | Aucun appelant oublie | TypeScript a designe les 3 constructeurs de contexte (ecran, banc d'essai, test de boucle) — c'est le compilateur qui a fait le recensement, pas ma memoire | OK |
| B-039 | L'apercu au vocabulaire du reste | Cadre PLEIN teinte d'accent au lieu du pointille, titre « Ce que tu vas enregistrer » au lieu d'« Apercu » (`RegleFormDialog`). Le pointille signalait un etat vide partout ailleurs (8 occurrences) | OK |
| B-039/B-040 | Preuves | `npx tsc --noEmit` → 0 erreur · `npm test` → 1373 passed · `npm run build` → Compiled successfully | OK |
| B-039/B-040 | Non couvert, dit franchement | Aucun test ne garde le rendu de l'apercu ni la formulation du prompt : les deux se jugent a l'oeil et a l'usage. **Non verifie automatiquement** | partiel |
