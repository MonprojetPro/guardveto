...
|---|---|---|---|---|
| B-043 | **⚠️ Le produit parle de matin, apres-midi et journee — alors qu'il ne planifie QUE les soirs et les week-ends.** Releve par MiKL a plusieurs endroits le 26/08. **Deux defauts distincts, meme motif : un parametre montre que le moteur n'evalue pas.** **(A) LES REGLES** — `periodeLisible` traduit `apres_midi` en « l'apres-midi » et la forme TABLEAU l'affiche toujours (`catalogue.ts:139`). Or le code dit lui-meme que `periode: 'apres_midi'` **n'a JAMAIS ete evalue** : la regle bloque le JOUR ENTIER. Le correctif de 2026-08 avait retire cet affichage de la forme SIMPLE et **oublie la forme TABLEAU** — c'est-a-dire precisement celle d'Anne-Sophie (« jeudi AP impaires + lundi AP paires + mercredi paires »). Elle lit donc une portee partielle que son planning n'applique pas. **(B) LES CONGES** — le formulaire propose `journee | matin | apres-midi | soiree` (`CongeForm.tsx`), et `checkR16Conge` (hard-constraints.ts:506) **ignore totalement le creneau** : n'importe quel conge bloque la journee entiere. Une veterinaire qui pose une matinee perd sa garde du soir, sans que rien ne le dise. **Latent aujourd'hui : mesure en base, les 32 conges des deux cabinets ont `creneau = null`** — personne ne s'en est encore servi. **Question produit a trancher** : puisque seuls les soirs et week-ends sont planifies, le seul creneau qui aurait un effet est « soiree ». Retire-t-on le vocabulaire demi-journee (conges et libelles de regles), ou l'implemente-t-on ? | MiKL | 2026-08-26 | A qualifier |
