...
| B-048 | **⚠️ « L'equipe GuardVeto a recu ta situation » est affiche SANS AUCUNE PREUVE D'ENVOI.** Trouve le 26/08 en repondant a MiKL (« il va ou le message ? je n'ai rien recu »). `signalerLimite` (`assistance/actions.ts:106`) fait `await sendBrevoEmail(...)` dans un try/catch — mais **`sendBrevoEmail` ne leve JAMAIS** : elle RETOURNE `{ error }` (expediteur manquant, cle absente, rejet Brevo). La valeur de retour n'est pas lue. Le catch n'attrape donc rien et l'ecran affiche « C'est parti » dans tous les cas. ⚠️ Le destinataire est `contact@monprojet-pro.com` — precisement l'adresse que Brevo REJETAIT le 21/08 (« sender not valid ») sur d'autres envois. Et le signalement **ne journalise rien dans `email_log`** : aucune ligne le 26/08 a 18h49 (verifie en base). On ne peut donc meme pas savoir apres coup si un message est parti. Meme famille que « une RPC absente + un await qui n'ecoute pas » (21/08) et que B-031. | MiKL | 2026-08-26 | Correctif — a traiter avant toute recette du bouton |
| B-049 | **Le rapport d'impasse est un dump technique, pas une explication.** MiKL, 26/08 : « ouah le truc horrible !!!!! ». L'ecran empile des codes machine (`R16 : ...`), des comptages bruts (« bloque 32 creneaux »), dix regles a plat sans hierarchie, trois « pistes » qui repetent la meme phrase, puis une liste de 25 creneaux date par date. Deja consigne le 19/08 : **un bandeau est un SIGNAL, pas un rapport** — grouper par CAUSE, jamais un code machine. La regle existe, l'ecran ne la suit pas. ⚠️ **Pire que moche : le chiffre est FAUX** (trouve le 26/08 en repondant a MiKL). « 25 creneaux non couverts » n'est pas un nombre de problemes — `joursNonCouverts = steps.slice(indexImpasse)` (`solver.ts:777`) renvoie **tout ce qui suit le point d'arret**. Un seul blocage rougit donc trois semaines de calendrier, et l'admin croit son planning irrecuperable alors qu'il bute sur un seul enchainement. **Un ecran qui compte faux est plus grave qu'un ecran illisible.** | MiKL | 2026-08-26 | Refonte de l'ecran d'impasse |
| B-050 | **« Prevenir l'equipe » envoie un message vide et n'apprend rien a personne.** MiKL, 26/08 : « s'il ne met rien dans le message, comment je sais moi ce qui ne va pas ? ». Le champ libre est facultatif : un signalement peut partir sans une ligne d'explication. Demande de MiKL : **orienter vers l'onglet Assistance**, en disant de prendre une capture d'ecran AVANT et de decrire la situation — au lieu d'un envoi a l'aveugle depuis l'ecran de generation. A traiter avec B-048 (aujourd'hui, meme le message vide ne part peut-etre pas). | MiKL | 2026-08-26 | Parcours d'assistance |
