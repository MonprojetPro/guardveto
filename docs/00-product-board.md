...
|---|---|---|---|
| B-038 | Choix « toutes / paires / impaires » a la saisie | Champ « Quelles semaines ? » sur la regle « Repos fixe un jour » (`RegleFormDialog`), etat relu depuis la forme tableau a l'edition | OK |
| B-038 | La forme ecrite est celle que les gardiens lisent | `construireParams` produit `regles: [{jour, semaine}]` des qu'une parite est choisie — la SEULE forme evaluee avec parite par `violeReposFixe` et `validerPlanning` | OK |
| B-038 | Le moteur applique REELLEMENT (pas seulement la forme) | 3 epreuves bout-en-bout dans `tests/lib/repos-fixe-semaine-sur-deux.test.ts` : refuse le jeudi impair, ACCEPTE le jeudi pair (c'est ce second point qui prouve que la parite est lue), et sans parite refuse les deux | OK |
| B-038 | Aucun parametre montre sans effet | « Sauf vacances scolaires » n'est lu par les gardiens que sur la forme simple : la case DISPARAIT quand une parite est choisie, et le serveur REFUSE la combinaison au lieu de l'ignorer. Le defaut `periode: 'apres_midi'` ne se rejoue pas | OK |
| B-038 | Filou sait la creer | `regleSchema` (champ `semaine`, payload, apercu) + description du type 1 dans le prompt, avec la mise en garde contre la confusion avec le type 3 | OK |
| B-038 | Phrase lisible | Un seul jour se dit « ne fait pas de garde le jeudi (semaines impaires) » et non « a des repos fixes : … », qui se lisait comme une liste amputee (`catalogue.ts`) | OK |
| B-038 | Non-regression des regles existantes | « Toutes les semaines » produit la forme simple a l'identique, exception vacances comprise — epreuve dediee | OK |
| B-038 | Preuves | `npx tsc --noEmit` → 0 erreur · `npm test` → 1373 passed (contre 1365 : +8) · `npm run build` → Compiled successfully | OK |
...
|---|---|---|---|---|
| B-038 | **Repos fixe « une semaine sur deux » — saisissable a l'ecran et par Filou.** Demande de MiKL en seance. **Les DEUX gardiens savaient deja l'evaluer** (forme heritee du cabinet pilote) : seule la saisie manquait, donc aucun code moteur n'a ete ecrit. Piege ecarte au passage : « sauf vacances scolaires » n'etant lu que sur la forme simple, la case disparait avec une parite et le serveur refuse la combinaison — plutot que d'afficher un assouplissement que le planning n'applique pas. **A RECETTER par MiKL.** | `a venir` | 2026-08-26 | A qualifier |
