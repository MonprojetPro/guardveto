...
|---|---|---|---|---|
| B-038 | **Pouvoir poser un jour de repos une semaine sur deux (paires / impaires).** Demande de MiKL le 26/08 en seance. **Mesure faite avant de chiffrer : les DEUX gardiens savent deja le faire.** `violeReposFixe` (hard-constraints.ts) et `validerPlanning.ts` evaluent tous deux la forme « tableau de regles » `regles: [{jour, semaine: paire|impaire, ancre}]` — c'est ainsi qu'est stockee la regle heritee d'Anne-Sophie. Ce qui manque est **uniquement la SAISIE** : le formulaire (`RegleFormDialog` + `paramsRegle`) et Filou (`regleSchema`) ne construisent que la forme simple `{ jour }`, sans parite. Aujourd'hui on peut donc dire « pas de garde les semaines impaires » (tous les soirs, via `alternance_ancre`) ou « pas de garde le jeudi » (toutes les semaines), mais pas le croisement — sauf a ecrire le JSON a la main. ⚠️ **Piege connu a ne pas rejouer** : sans `ancre`, la parite retombe sur la parite ISO globale, d'ou le bug de la semaine 53 deja paye (correctif F7-001). L'ancre doit etre posee, vraisemblablement au debut de periode. | MiKL | 2026-08-26 | A qualifier |
