...
| B-039 | **Le bloc « Apercu » de la fenetre des regles n'est pas au vocabulaire du reste.** Signale par MiKL le 26/08 « au passage ». Constat verifiable : il utilise `border-dashed` + `bg-muted/40`, or dans TOUT le reste du produit le cadre pointille signale un ETAT VIDE (« aucune regle », « aucun echange », « aucune notification » — 8 occurrences). Ici il encadre du contenu reel, et le plus important de la fenetre : **la phrase sur laquelle l'admin valide**. Le cadre dit « rien ici » a l'endroit ou il faudrait lire « voila ce que tu t'appretes a ecrire ». A traiter avec PIXEL. | MiKL | 2026-08-26 | A qualifier |
...
|---|---|---|---|
| B-038a | L'apercu affiche la parite choisie | `parite` ajoutee aux dependances du `useMemo` de l'apercu (`RegleFormDialog`). Elle manquait : le champ marchait, la regle s'enregistrait correctement, seul l'APERCU restait fige sur la phrase sans parite | OK |
| B-038a | Pourquoi personne ne l'a vu | Le `eslint-disable react-hooks/exhaustive-deps` pose sur cette ligne desactive exactement le controle qui l'aurait signale. Commentaire ajoute au-dessus de la liste pour que la prochaine variable ajoutee n'y echappe pas | OK |
| B-038a | Preuves | `npx tsc --noEmit` → 0 erreur · `npm test` → 1373 passed, inchange (l'apercu est du rendu React, hors perimetre des epreuves pures de B-038) | OK |
| B-038a | Non couvert, dit franchement | Aucun test ne garde ce raccord : il faudrait monter le composant. Le defaut se reverra a l'oeil, comme MiKL vient de le faire. **Non verifie automatiquement** | partiel |
